...
- **命名**：统一 `sk-*`
- **数据源**：统一走 **Gangtise OpenAPI**（行情/财务/估值/一致预期/股东/主营/公告/AI 能力），外部搜索保留 **Tavily**，通用兜底 WebSearch/WebFetch。详见 `core/adapters.md`
...
- **三层懒加载**：Tier 0 启动必读（`_boot` + `.task-pulse` + `CLAUDE.md`，≈1.5k token）→ Tier 1 调 skill 时加载（SKILL + preamble + postamble + adapters）→ Tier 2 按需（evidence / compliance / acceptance）
- **强制流程**：任何 skill 调用前走 `core/preamble.md`（识别市场 → 查历史输出 → Preflight 取数计划 → 实际取数），结束走 `core/postamble.md`（证据自检 → 仍需补的资料 → 合规声明 → 归档 → 验收）
- **证据分级**：每条关键事实带中文标签；关键结论必须由 `公开事实`/`财报披露` 支撑，`待核验假设` 不能作为结论唯一依据
- **双输出纪律**：对话贴完整内容（云端用户直读）+ 同时写文件归档，不只回摘要
- **断点续跑**：长任务每完成一段写 `.checkpoint`，新会话用"继续 {task-id}"恢复
...
# 验证（本 harness 基于 v0.17.0 验证，2026-06-15）
gangtise --version
...
```
1. Gangtise OpenAPI   全市场主数据源（A股/港股/美股；代码带 .SH/.SZ 后缀，如 600000.SH / 000001.SZ）
