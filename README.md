...
一条命令静态校验 15 项：skill/agent 注册对齐（含重复注册）、SKILL frontmatter、版本戳四处一致 + CLI 对齐日期三处一致（VERSION / manifest / adapters / README）、本机 CLI 版本 vs 文档基线、字母证据缩写残留、覆盖池互斥（coverage.md 当前覆盖 ∩ watchlist.md 当前观察）、coverage/ 目录归属、`.task-pulse` JSON、checkpoint ↔ `.task-pulse` 双向一致、状态文件占位符、私密数据不入库（git 索引 + .gitignore 策略双检）、归档命名、产物 frontmatter（必填项 + 值合法 + 覆盖周报/月报/健康检查等非日期前缀产物）、内部链接。⚠️ 警告不拦截；❌ 失败退出码 1。**框架改动或 CLI 版本对齐后必跑。**

改过 `scripts/harness-doctor.sh` 本身后，再跑固定反例回归 `bash scripts/harness-doctor-tests.sh`——24 组用例 / 28 项断言，把工作区复制到临时目录逐例注入异常再断言 doctor 的反应：版本日期漂移、观察池历史区不误报、历史区标注不替当前行背书、缺状态文件（watchlist/coverage）、manifest 重复注册、pulse 指向缺失 checkpoint、产物 frontmatter 缺失/空值/未闭合/非枚举/非法日历/重复键、周报·月报·健康检查非日期前缀产物覆盖（含 ISO 周历史门槛 W14 跳过、根目录 health-check 防逃逸）、.gitignore 保护缺失。依赖私密 live 文件的用例在文件缺失时 SKIP，公开仓库克隆后也能跑。
...
├── scripts/harness-doctor.sh      # 静态自检（15 项，见「自检」节）
├── scripts/harness-doctor-tests.sh # doctor 反例回归（24 组用例 / 28 断言）
│
...
数据层依赖 **[gangtise-openapi-cli](https://github.com/gangtiser/gangtise-openapi-cli)**（命令行 `gangtise`，提供实时/历史行情 / 财务三表（A股·港股·美股）/ 估值 / 一致预期 / 股东 / 主营 / 资金流向 / 公告 / 产业公众号 / 投资者问答 / 研报图表 / 证券级数据指标（EDE，A股·港股·美股）/ AI 能力）。它是主数据源——没有它，只能退到 Tavily / WebSearch 兜底。

```bash
# 安装（需 Node.js 20+）
npm install -g gangtise-openapi-cli

# 验证（本 harness 基于 v0.27.0 验证，2026-07-11）
gangtise --version
...
- **运行约束（v0.23+）**：自动翻页接口省略 `--size` 会拉全量；日常列表必须显式 `--size N`，未知量级先用 `--size 1` 探 `Total`
- **计费安全（v0.26+）**：`one-pager` / `knowledge-batch` 等 13 个贵档端点失败**不自动重放**（按次计费、缓存命中不豁免）；失败先判因再决定是否手动重试，EDE `999999` = 查询无数据而非故障
- **partial 结果**：遇到退出码 3 或 JSON 里的 `partial: true`，只能标为"部分结果 / 待补拉"，不得当成完整取数（v0.23 起也覆盖 `fund-flow` / 分钟 K / 多标的日 K 撞 `--limit` 的截断场景）
- **下载 / 输出**：`download --output` 会跟随跳转并实际落文件（v0.26 起原子写、坏跳转直接报错不存 HTML）；仍需检查目标路径和文件大小
- **代码格式**：证券代码须带交易所后缀（A 股 `.SH` / `.SZ` / `.BJ`，港股 5 位 + `.HK`，美股 `.O` / `.N` / `.A`），裸代码会报错或返回空
- **调用约定**：取数口径以 `core/adapters.md` 为唯一事实源——换 CLI 版本先改 adapters；个别盘面/批量类 skill 内嵌的命令示例须随 adapters 对齐（版本升级时一并核对）
