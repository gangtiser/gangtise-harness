...
| 编号 | Skill | 说明 |
|---|---|---|
| 1 | sk-company-deepdive | 公司深度研究 |
| 2 | sk-industry-map | 行业图谱 |
| 3 | sk-thesis | 投资命题构建 |
| 4 | sk-earnings-preview | 财报前瞻 |
| 5 | sk-model-check | 财务模型校验 |
| 6 | sk-consensus-watch | 一致预期监控 |
| 7 | sk-catalyst-monitor | 催化剂跟踪 |
| 8 | sk-roadshow-questions | 路演问题设计 |
| 9 | sk-red-team | 反方论证 |
| 10 | sk-pm-brief | PM 一页纸 |
| 11 | sk-briefing | 投研简报 |
| 12 | sk-batch-refresh | 覆盖池批量刷新 |
| 13 | sk-batch-earnings | 财报季批量前瞻 |
| 14 | sk-catalyst-sweep | 每日催化剂扫描 |
| 15 | sk-autopilot | 自动路由 |
| 16 | sk-master | 全能 7 模式 |
| 17 | sk-tape-review | 盘面 + 技术面复盘 |
| 18 | sk-deck-builder | PPT / 路演材料生成 |
| 19 | sk-stock-screen | 条件选股 / 找标的 |
| 20 | sk-industry-database | 行业 / 公司数据库搭建 |
| 21 | sk-close-recap | 收盘复盘 |
| 22 | sk-hourly-watch | 盘中小时监控 |
| 23 | sk-daily-feed | 日度信息流 |
| 24 | sk-people-watch | 关键人物 / 社区跟踪 |
| 25 | sk-question-list | 路演 / 调研问题清单 |
| 26 | sk-qa-archive | Q&A / 纪要归档 |
| 27 | sk-wiki-build | 研究 wiki 构建 |
| 28 | sk-health-check | 覆盖池健康检查 |
...
- 出现公司名 + "深度 / 起 coverage / 看看" → sk-company-deepdive
- 出现行业名 + "看看 / 怎么看" → sk-industry-map
- 出现 "财报 / 业绩 / 前瞻" → sk-earnings-preview
- 出现 "反方 / 唱反调 / 空头 / 找问题" → sk-red-team
- 出现 "晨会 / 简报 / 整理" → sk-briefing
- 出现 "PM / 一页纸 / 给老板" → sk-pm-brief
- 出现 "调研 / 路演 / 业绩会问题" → sk-roadshow-questions
- 出现 "模型 / 估值 / DCF" → sk-model-check
- 出现 "一致预期 / 预期差" → sk-consensus-watch
- 出现 "催化 / 事件 / 政策" → sk-catalyst-monitor
- 出现 "刷新覆盖池 / 周度更新" → sk-batch-refresh
- 出现 "财报季 / 一周财报" → sk-batch-earnings
- 出现 "扫一遍 / 今日催化" → sk-catalyst-sweep
- 出现 "盘面 / K 线 / 技术面 / 复盘走势" → sk-tape-review
- 出现 "PPT / deck / 路演材料 / IC pitch" → sk-deck-builder
- 出现 "选股 / 筛标的 / 挖标的 / 哪些票值得看" → sk-stock-screen
- 出现 "数据库 / 底表 / EDB / 产业库 / 指标库" → sk-industry-database
- 出现 "收盘复盘 / 盘后复盘 / 今天为什么涨跌" → sk-close-recap
- 出现 "盯盘 / 看盘 / 盘中异动 / 小时监控" → sk-hourly-watch
- 出现 "日度信息流 / 每日跟踪 / 信息流" → sk-daily-feed
- 出现 "关键人物 / 大 V / 产业号 / 社区情绪" → sk-people-watch
- 出现 "问题清单 / 要问什么 / 调研提纲" → sk-question-list
- 出现 "Q&A 归档 / 纪要归档 / 调研归档" → sk-qa-archive
- 出现 "wiki / 研究底稿 / 知识库搭建" → sk-wiki-build
- 出现 "健康检查 / 覆盖池体检 / 数据完整性" → sk-health-check
- 完全模糊 / 没线索 → sk-autopilot
...
🎯 用 sk-company-deepdive 起 coverage：{你填的标的}
