...
- **代码必带交易所后缀**：A股沪市 `.SH`、深市 `.SZ`、港股 `.HK`、美股 `.US`（如 `600000.SH` / `000001.SZ`）。裸代码在 `fundamental` 报 `430009 非有效A股`、在 `quote` 返回空。
- **不确定代码 / 只知公司名** → 先查：`gangtise reference securities-search`（名称/简称 → 代码），或 `gangtise lookup` 查券商/会议机构 ID。
...
| 命令 | 用途 |
|---|---|
| `day-kline` / `day-kline-hk` / `day-kline-us` | A股 / 港股 / 美股 日 K（`--security <code> --start-date --end-date --limit --field --format`） |
...
调用范式：security 类（one-pager / investment-logic / peer-comparison / earnings-review / research-outline 等）= `--security-code <code> --format json`；`knowledge-batch` = `--query "{标的} {关键词}" --resource-type 10 --resource-type 60 --top 15`。其余精确参数见 `gangtise ai <cmd> --help`。
...
gangtise quote day-kline --security {code} --limit 5 --field close --field pctChange --format json   # 港股用 day-kline-hk，美股 day-kline-us
```
...
gangtise ai security-clue --security-code {code} --format json                              # 异动 / 卖方线索
```
...
| **§H 港股** | Gangtise（`quote day-kline-hk`、`fundamental *-hk`、`insight announcement-hk`、AI、知识库）→ `.cache` → Tavily → WebFetch HKEX → 兜底 |
| **§U 美股** | Gangtise（`quote day-kline-us`、AI、知识库；**无三表财务**）→ `.cache` → Tavily → WebSearch SEC → WebFetch EDGAR → 兜底 |
