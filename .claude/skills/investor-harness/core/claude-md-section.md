...
| 我说什么 | 你做什么 |
|---|---|
| "看一下 X" / "X 怎么样" | 走 `sk-autopilot` 自动路由 |
| "深度看 X" / "起 coverage" | 走 `sk-company-deepdive` |
| "X 财报前瞻" | 走 `sk-earnings-preview` |
| "反过来想 X" / "X 空头逻辑" | 走 `sk-red-team` |
| "X 预期差" | 走 `sk-consensus-watch` |
| "整理今天的 X" / "晨会" | 走 `sk-briefing` |
| "给 PM 一页纸" | 走 `sk-pm-brief` |
| "X 行业框架" / "X 产业链" | 走 `sk-industry-map` |
| "怎么问 X 管理层" | 走 `sk-roadshow-questions` |
| "看 X 的 K 线" / "复盘 X" | 走 `sk-tape-review` |
| "做 X 的 PPT" / "生成 deck" / "IC pitch" / "路演材料" | 走 `sk-deck-builder` |
...
`sk-master` · `sk-autopilot` · `sk-thesis` · `sk-industry-map` · `sk-company-deepdive` · `sk-earnings-preview` · `sk-model-check` · `sk-consensus-watch` · `sk-catalyst-monitor` · `sk-roadshow-questions` · `sk-red-team` · `sk-pm-brief` · `sk-briefing` · `sk-tape-review` · `sk-deck-builder` · `sk-batch-refresh` · `sk-batch-earnings` · `sk-catalyst-sweep` · `sk-stock-screen` · `sk-industry-database` · `sk-close-recap` · `sk-hourly-watch` · `sk-daily-feed` · `sk-people-watch` · `sk-question-list` · `sk-qa-archive` · `sk-wiki-build` · `sk-health-check`
...
| `{{HARNESS_PATH}}` | harness 安装路径 | `.claude/skills/investor-harness`  |
| `{{WORKSPACE_ROOT}}` | 用户工作区路径 | `~/investor-research`  |
| `{{COVERAGE_ROOT}}` | 覆盖库路径（通常是 `{WORKSPACE_ROOT}/coverage`） | `~/investor-research/coverage` |
| `{{DATA_SOURCES}}` | 逗号分隔的数据源列表 | `gangtise-openapi,websearch` |
| `{{DATA_SOURCES_PRIORITY_CHAIN}}` | 多行的优先级链描述 | 见下方 |

### `{{DATA_SOURCES_PRIORITY_CHAIN}}` 示例

用户选了 Gangtise OpenAPI+ websearch：
...
**港股**:
  1. Gangtise OpenAPI 
  2. WebFetch https://www.hkexnews.hk/
  3. WebSearch
  4. 用户贴材料

**美股**:
  1. Gangtise OpenAPI
  2. WebSearch site:sec.gov
  3. WebFetch sec.gov
  4. 用户贴材料
```
