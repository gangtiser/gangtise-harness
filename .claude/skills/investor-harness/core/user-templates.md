...
>
> 这是"任务永久化"三层的**第一层 L1**：模板层（复用 sk-* skill，自定义输出结构和归档路径）。
...
based_on_skill: sk-briefing      # 基于哪个 sk-* skill（必填）
...
如果模板指定了 `based_on_skill: sk-briefing`，LLM 加载 `sk-briefing/SKILL.md` 作为**执行框架**。
...
严格按用户 `## 输出结构` 段的定义。如果用户写了"5 段"而不是 sk-briefing 默认的 5 段，听用户的。
...
- `based_on_skill:` 明确指定一个 sk-* skill（不能空）
...
- `输出结构` 抄 sk-briefing 原文（那还不如直接用 sk-briefing）
- 试图在模板里绕过合规 / 证据分级（core/ 的硬约束不能 override）
