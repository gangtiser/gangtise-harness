...
> v0.7 新增。L2 = 在现有 sk-* 基础上扩展（继承）。L3 = 完全自创新 skill。
>
...
{workspace_root}/
├── user-skills/
│   ├── my-deepdive-esg/            ← L2: 继承 sk-company-deepdive
│   │   └── SKILL.md
...
extends: sk-company-deepdive        # 必填：被继承的 sk-* skill
description: 在标准公司深度基础上增加 ESG 专项段
inputs:
  - 公司名 / 代码
  - 可选：ESG 评级数据
outputs:
  - 继承 sk-company-deepdive 的 9 段 + §9.5 ESG 专项
data_sources: 见 .claude/skills/investor-harness/core/adapters.md + MSCI ESG / 中证 ESG
markets: [CN-A, HK, US]
---
...
本 skill 继承 `sk-company-deepdive` 的全部结构和流程。
...
**修改 / 新增的**：
- 在 §8（未来三个月跟踪指标）和 §9（仍需补的资料）之间，**插入** §8.5 "ESG 专项"
...
- ESG 数据必须带证据等级，MSCI / 中证 / 公司自披露分别标 财报披露 / 市场共识 / 合理推演
...
3. 看到 `extends: sk-company-deepdive` → 同时加载父 skill
4. **合并规则**：
...
### 2. 证据分级必备
每条事实必须带 公开事实/财报披露/市场共识/合理推演/待核验假设 标签，和主 skill 一样。

### 3. "仍需补的资料"段非空
最后一段必须是"仍需补的资料"，分必需 / 建议 / 不确定三档。

### 4. 合规声明
末尾必须有合规声明，不承诺收益，评级目标价需人工复核。

### 5. 归档协议
输出必须写到文件，对话 + 文件双输出，按命名规范。
...
# 1. 从 user-skills 复制到 skills/，加 sk- 前缀
cp -r ~/workspace/user-skills/my-hk-ipo-analysis skills/sk-hk-ipo-analysis

# 2. 更新 SKILL.md 的 name 字段
# name: my-hk-ipo-analysis → name: sk-hk-ipo-analysis
...
# 8. 提 PR
git checkout -b feature/sk-hk-ipo-analysis
git add skills/sk-hk-ipo-analysis core/_boot.md manifest.yaml README.md core/acceptance.md
git commit -m "feat: add sk-hk-ipo-analysis skill (contributed by {your name})"
git push origin feature/sk-hk-ipo-analysis
gh pr create
```
...
回流后 sk- 前缀表示"官方承认"，L3 的 my- 前缀表示"用户自用"。
...
- ❌ 不能"继承 sk-company-deepdive 但去掉 §9 仍需补的资料"
- ✅ 可以"在 §8 后新增 §8.5 ESG"
...
### 陷阱 3：L3 不引用 core/
- ❌ skill 里所有规则自己重新写一遍
- ✅ 引用 core/ 的文件（`.claude/skills/investor-harness/core/preamble.md`），继承规则
...
- ❌ `user-skills/sk-company-deepdive/`
