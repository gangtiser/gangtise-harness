...
> v0.7.0 新增。本文件是 Librarian 模式的总览。每一节都对应一个独立的 core/*.md 详规和一个 sk-* skill。
...
| 能力 | 详规 | 配套 skill |
|---|---|---|
| 1. 每日信息流（Daily Feed）：活的公司主页 | [wiki-architecture.md](wiki-architecture.md) + [daily-feed.md](daily-feed.md) | `sk-daily-feed`、`sk-wiki-build` |
| 2. analyst-level 任务跟踪：Question List + vault 扫描 | [qa-double-link.md](qa-double-link.md) | `sk-question-list` |
| 3. Q&A 双链复利 | [qa-double-link.md](qa-double-link.md) | `sk-qa-archive` |
| 4. 健康检查与跨源仲裁：让 wiki 自己维护自己 | [health-check.md](health-check.md) | `sk-health-check` |
...
未明示则按原有 17 个 sk-* skill 的规则工作（参见 [_boot.md](_boot.md)）。
