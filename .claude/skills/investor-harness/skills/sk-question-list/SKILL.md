---
name: sk-question-list
description: 见分析师 / 管理层 / 产业专家之前，生成 question list 文件并触发 vault 扫描，每个问题下面自动附"vault 扫描初步结论"——交叉综合现有的所有纪要、研报、公众号、模型数据，列出当前知识库已经能回答的部分、已知的矛盾点、以及建议的追问方向。打印出来就是完整的会前 briefing。
inputs:
  - ticker / 公司名
...
Q1: FY26 出海销量目标？
  ...
  - [[model FY26]] 当前假设 32k 单位（A 级 - 财报披露 + 自己模型）
