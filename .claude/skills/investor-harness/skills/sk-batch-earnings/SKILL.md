...
Batch Earnings 特别注意：每家公司的子任务必须**完整调用 `sk-earnings-preview`** 的逻辑，不能简化。preamble Step 4 必须先列出本次时间窗内的全部财报日历。
...
- 实际 vs 你的 preview
- 对原 thesis 的影响
- 是否需要触发 sk-thesis update

## 输出验收（除通用清单外）

- [ ] 财报日历完整且按日期排序
- [ ] 每家公司的 preview / postmortem 都通过 sk-earnings-preview 的专属验收清单
- [ ] 行业层面观察非空
- [ ] 高优先级标的有显式标注
...
- **每家 preview 内部**：触发 `sk-consensus-watch` 拿一致预期
- **postmortem 后**：如果实际显著偏离 preview → 触发 `sk-thesis update`
- **若高优先级**：自动建议跑 `sk-pm-brief` 给 PM 一页纸
- **批量收尾**：自动建议跑 `sk-briefing` 整合成晨会要点
...
- [../sk-earnings-preview/SKILL.md](../sk-earnings-preview/SKILL.md)
