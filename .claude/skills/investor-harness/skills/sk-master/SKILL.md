---
name: sk-master
description: 二级市场投研总控 skill（长形态）。7 种模式 Thesis/Coverage/Consensus/Catalyst/Red Team/Briefing/PM Prep 的全量说明，适合当作"单一 skill 入口"独立使用。若已安装完整 Investor Harness 套件，优先用 sk-autopilot 做路由，再进入各专门 skill。
inputs:
  - 公司名 / 股票代码 / 行业 / 主题
...
这是 Investor Harness 的**长形态总控 skill**。如果你只想装一个 skill 就能跑全套流程，用这个。如果你装了完整套件（12 个专业子 skill），请优先使用 `sk-autopilot` 路由到具体子 skill。
...
见 [../../core/evidence.md](../../core/evidence.md)。输出时为关键事实打完整中文证据标签。**不要**把 `待核验假设` 当成结论核心。
