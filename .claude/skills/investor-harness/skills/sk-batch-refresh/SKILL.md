...
| Ticker | 名称 | 失败原因 |
|---|---|---|
| {ticker} | {name} | Gangtise OpenAPI CLI 超时 / 数据未披露 |
```
...
## 行情快照
- 收盘价 / 周涨跌 / 月涨跌 / YTD：按市场选择 `day-kline` / `day-kline-hk` / `day-kline-us`，传对应 `--start-date` / `--end-date` 窗口后按 `tradeDate` 取尾部，不用裸 `--limit`

## 财务最新
- 最近一期 PE / PB / PS / ROE：`gangtise fundamental valuation-analysis --security-code {ticker} --indicator peTtm --indicator pbMrq --format json`
- 营收 / 净利 / 毛利率 同比：`gangtise fundamental income-statement --security-code {ticker} --field totalOpRev --field netProfit --field netProfitAttrParent --format json`

## 股东变化
- 十大股东最新一期：`gangtise fundamental top-holders --security-code {ticker} --holder-type top10 --format json`
- 与上一期对比 (合理推演)
...
- Gangtise OpenAPI CLI 有调用配额限制（错误码 903301 为今日上限），超过需分批或次日续跑
...
### 任务持久化
- 长批量任务必须支持中断恢复
- 在 `.task-pulse` 写简要进度 + `.checkpoint/{task-id}.md` 记"已完成 N/总 M"明细，断点继续

## 与其他 skill 的协作

- **触发深度研究**：如果某只标的检测到"显著变化"，**自动**触发 `sk-catalyst-monitor` 单点分析
- **触发反方审视**：如果某只标的本周涨幅超过 20%，**自动建议**走 `sk-red-team`
- **触发命题更新**：如果财务数据与上一份 thesis 假设矛盾，**主动提醒**用户跑 `sk-thesis` update
