...
Consensus Watch 特别注意：必须重点获取卖方一致预期（`gangtise fundamental earning-forecast --security-code {ticker} --consensus netIncome --consensus eps --format json` 或卖方研报摘要），缺这层数据时**禁止**输出预期差判断，必须明确告知用户"无法判断预期差"并走兜底。
...
- 不要脱离时间窗口谈预期差
- 尽量写清比较基准和估值语境
- 市场已反映的预期标 `市场共识`，被低估变量的支撑证据标 `公开事实`/`财报披露`
