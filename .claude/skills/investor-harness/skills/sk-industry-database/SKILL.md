---
name: sk-industry-database
description: 产业数据库搭建 skill。用于把公开市场数据、公司披露、卖方报告、行业数据库（如 EDB / Gangtise OpenAPI EDB / 第三方产业库）整理成可追溯的 Excel 数据库，既支持产业数据库，也支持公司数据库、指标库和长期跟踪底表。
inputs:
  - 行业 / 赛道 / 公司 / 指标范围
...
| `source_date` | 源材料日期 |
| `captured_at` | 本次入库日期 |
| `evidence_label` | 公开事实 / 财报披露 / 市场共识 / 合理推演 / 待核验假设 |
...
这张 sheet 专门把证据等级写成**完整中文**，不再使用字母缩写：

| 证据等级 | 说明 | 能否作为核心结论依据 |
|---|---|---|
| `公开事实` | 公开、稳定、可直接验证的事实 | 可以 |
| `财报披露` | 财报、公告、官方披露、权威数据库中的明确数字或事实 | 可以 |
| `市场共识` | 卖方一致预期、市场普遍看法、可验证的共识口径 | 只能辅助 |
| `合理推演` | 基于已知事实做出的推导或换算，必须写出链路 | 只能辅助 |
| `待核验假设` | 传闻、渠道、初步线索或尚未核验的判断 | 不能单独作结论 |
...
1. 公司公告 / 年报 / 季报 / 业绩会
2. 权威行业数据库 / EDB / Gangtise OpenAPI EDB / 第三方产业数据库
3. 公开市场数据
4. 卖方报告
5. 自行测算
...
| 关系 | 说明 |
|---|---|
| **上游** | 可先用 `sk-industry-map` 定义产业链框架 |
| **平行** | `sk-company-deepdive` 负责写研究，`sk-industry-database` 负责搭长期底表 |
| **下游** | 数据库可以继续供 `sk-thesis` / `sk-consensus-watch` / `sk-pm-brief` 调用 |
