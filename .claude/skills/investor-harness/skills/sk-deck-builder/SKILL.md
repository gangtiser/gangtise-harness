...
### Slide 5 · 支撑证据 (Evidence)

一页放 3-5 个最硬的数据点，每个**必须**带证据等级：
...
| 关系 | skill | 说明 |
|---|---|---|
| **强依赖** | `sk-company-deepdive` | 必须至少读到一份 deepdive 才能做 ic-pitch |
| **强依赖** | `sk-thesis` | 命题来源 |
| **强依赖** | `sk-red-team` | Slide 8 内容必须来自这里 |
| **强依赖** | `sk-earnings-preview` | earnings-review deck 的数据源 |
...
**推荐流程**：
```
sk-thesis → sk-company-deepdive → sk-red-team → sk-consensus-watch
                                                          ↓
                                                sk-deck-builder
                                                          ↓
                                                     IC pitch PPT
