...
- preamble Step 2 必须读取该公司最近一次 sk-company-deepdive 和 sk-thesis 输出，用于 §6 一致性检验
...
涨跌幅：±X.XX% (财报披露)
振幅：X.XX% (财报披露)
成交量：XX 万手 vs 60 日均量 XX 万手 → 量比 X.XX (财报披露)
换手率：X.XX% (财报披露)
区间表现（时间窗内）：±X.XX% (财报披露)
区间最大回撤：±X.XX% (财报披露)
```
...
| 维度 | 数据 | 含义 |
|---|---|---|
| 北向资金净买入（当日 / 5 日） | ±XX 亿 (财报披露) | 外资态度 |
| 主力净流入（当日） | ±XX 万 (财报披露) | 大单/特大单方向 |
| 龙虎榜（如上榜） | 买入/卖出席位 (财报披露) | 游资 vs 机构信号 |
| 融资余额变化 | ±X.X% (财报披露) | 杠杆资金信号 |
...
**K 线形态判断**（带证据）：
- {形态名，如"放量长阳"/"缩量十字星"/"长上影"/"破位阴线"} (公开事实-K 线观察)
...
**量价关系**：
- {量价齐升 / 价升量缩 / 缩量回调 / 放量破位 / 横盘缩量} (合理推演-基于成交量数据推演)
- 是否健康？
...
| 指标 | 当前值 | 信号 | 历史对比 |
|---|---|---|---|
| MACD (DIF/DEA/MACD) | XX/XX/XX (财报披露) | 金叉/死叉/0 轴附近 | vs 上次 |
| KDJ (K/D/J) | XX/XX/XX (财报披露) | 超买/超卖/中性 | — |
| RSI (6/14/24) | XX/XX/XX (财报披露) | 超买/超卖/中性 | — |
| BOLL (上轨/中轨/下轨) | XX/XX/XX (财报披露) | 价格位置 | 带宽变化 |
| ATR (14 日) | X.XX (财报披露) | 波动率 | vs 60 日均 |
| OBV | 趋势 (财报披露) | 量能背离/同步 | — |
...
**前置任务**：preamble Step 2 应读取该公司的最新 sk-company-deepdive 和 sk-thesis 输出。
...
| 关系 | 说明 |
|---|---|
| **依赖** | sk-company-deepdive / sk-thesis 的最新输出（用于 §6 一致性检验） |
| **被依赖** | sk-pm-brief 在涉及决策时机时可引用 tape-review 的关键位 |
| **互补** | 基本面 skill 给"为什么"，tape-review 给"什么时候"和"什么价格" |
| **冲突时** | 基本面命题与技术面强烈背离 → 触发 sk-red-team |
