...
outputs:
  ...
  - 触发首次跨源自检报告（待 sk-health-check 接力）
...
**Librarian 模式 skill**。仅当用户明示"建 coverage / 起 wiki page / onboard XXX"时启用，不在 sk-autopilot 默认路由内。
...
4. **填 14 段**：按 [wiki-architecture.md](../../core/wiki-architecture.md) 结构，每个数字带 wikilink 到原始文件 + 证据等级
5. **触发首次自检**：调用 sk-health-check 跑一遍跨源矛盾扫描，把发现的矛盾点写进 §14 仍需补的资料
