...
标的范围：覆盖池 82 家
催化类型：公告 / 新闻 / 价格异动 / 政策
预计调用：insight announcement list (按标的，近24h) + ai hot-topic (今日热点) + quote day-kline (价格异动)
```

### 第二步：分类型扫描

**类型 A：公司层公告**
- 对每家覆盖标的调 `gangtise insight announcement list --security {ticker} --start-time "{YYYY-MM-DD} 00:00:00" --end-time "{YYYY-MM-DD} 23:59:59" --rank-type 2 --format json`
...
**类型 C：价格异动**
- 对覆盖池每家标的调 `gangtise quote day-kline --security {ticker} --limit 2 --field close --field pctChange --format json`
